<template>
  <div class="flex grow flex-col gap-y-5 overflow-y-auto border-r border-gray-200 bg-slate-50 px-6 h-full">
    <div class="flex h-16 shrink-0 items-center">
      <img class="h-8 w-auto" src="https://tailwindui.com/img/logos/mark.svg?color=indigo&shade=600" alt="Your Company" />
    </div>
    <nav class="flex flex-1 mt-5 flex-col">
      <ul role="list" class="flex flex-1 flex-col gap-y-7">
        <li>
          <ul role="list" class="-mx-2 space-y-1">
            <li v-for="item in navigation" :key="item.name">
              <a :href="item.href" :class="[item.current ? 'bg-gray-200 text-indigo-600' : 'text-gray-700 hover:text-indigo-600 hover:bg-gray-50', 'group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold']">
...
<script lang="ts" setup>
      ...
      const navigation = [
        ...
        { name: 'Learn', href: '#', icon: CalendarIcon, count: '20+', current: false },
        { name: 'Create flashcards', href: '#', icon: UsersIcon, current: false },
        { name: 'Library', href: '#', icon: FolderIcon, count: '12', current: false },
        { name: 'Your progress', href: '#', icon: DocumentDuplicateIcon, current: false },
        { name: 'Test yourself', href: '#', icon: ChartPieIcon, current: false },
      ]
      const teams = [
