<template>
  <div
    ...
    :class="[
      reactiveStyles.dropableMenuTopSpacing,
      'text-gray-500',
      'w-screen',
      'absolute',
      'left-0',
    ...
    <div class="py-5 w-screen h-fit flex justify-center lg:pt-0 lg:w-fit">
      ...
      <div v-else class="px-8 lg:pt-5 lg:flex lg:space-x-12 w-fit">
        <RouterLink to="/signin">
          <span :class="reactiveStyles.navLinksTextColor"> Sign in </span>
        </RouterLink>
      </div>
    </div>
  </div>
...
import UserDropableMenu from '@/Components/Public/Navigation/UserNavigation/UserDropableMenu.vue'
import NavigationLinks from '@/Components/Public/Navigation/NavigationLinks.vue'
...
const isDarkHeroBackground = computed(() => {
...
const textStyleLogic = computed(() => {
  ...
  // if(isBrightBackground.value && isPageScrolled.value ) return false
  // if(!isBrightBackground.value && isPageScrolled.value ) return false
  if (isDarkHeroBackground.value && !isPageScrolled.value) {
    return true
  }
  // isDark === false &&
  return false
...
const checkIfNavShouldBeVisible = () =>
  getScreenWidth() >= 1024 ? uiStore.OPEN_NAVIGATION_MENU() : uiStore.CLOSE_NAVIGATION_MENU()
const checkIfUserNavShouldBeVisible = () =>
  getScreenWidth() >= 1024 && isSignIn ? uiStore.CLOSE_USER_MENU() : false
